...
export default function PayrollOverview() {
  ...
  return (
    <div>
      ...
      {/* Run Payroll panel — hamesha dikhega, chahe overview load ho ya fail ho */}
      <PayrollRunPanel month={month} year={year} />

      {overviewStatus === "loading" && (
        <Spinner full label="Loading payroll overview..." />
      )}

      {overviewStatus === "failed" && (
        <div className="card p-5 mb-6 flex items-center gap-2 text-rose-600 bg-rose-50 border-rose-100">
          <AlertCircle size={18} />
          <p className="text-sm">
            Payroll overview load nahi ho paayi. Backend API (<code>/payroll/overview</code>) check karo, ya "Run Payroll" se pehli baar payroll generate karo.
          </p>
        </div>
      )}

      {overviewStatus === "succeeded" && summary && (
        <>
